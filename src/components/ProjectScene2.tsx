...
export const ProjectScene2: React.FC<ProjectScene2Props> = ({
  ...
  const [activeChannel, setActiveChannel] = useState<'VOICE' | 'VISION' | 'EDGE'>('VOICE');
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let r = 0; r < rows; r++) {
        ...
        ctx.strokeStyle = r % 4 === 0 ? `rgba(53, 92, 255, ${alpha + 0.3})` : `rgba(234, 230, 220, ${alpha})`;
  ...
  return (
    ...
    >
      {/* Top Header */}
      <div className="flex items-baseline justify-between border-b border-white/10 pb-4 mb-12">
        <div className="flex items-baseline gap-4">
          <span className="font-display text-5xl sm:text-7xl md:text-8xl font-black text-white leading-none">
            02
          </span>
          <div className="flex flex-col">
            ...
            <span className="font-display text-xl sm:text-3xl font-bold tracking-tight text-white uppercase">
              {project2Data.title}
            </span>
          </div>
        </div>

        <div className="hidden lg:flex items-center gap-6 font-mono-tech text-xs text-white/50 tracking-widest uppercase">
      ...
      >
        ...
        <div className="absolute top-6 left-6 font-mono-tech text-[10px] sm:text-xs text-white/60 uppercase tracking-wider space-y-1 pointer-events-none">
          ...
          <div className="text-[#355CFF]">INTERACTION: MOVE CURSOR TO SIMULATE SENSOR FEED</div>
        </div>
          ...
          {(['VOICE', 'VISION', 'EDGE'] as const).map((channel) => (
            <button
              ...
              onClick={(e) => {
                ...
                audioEngine.playChime(channel === 'VOICE' ? 520 : channel === 'VISION' ? 780 : 960, 0.2);
              }}
      ...
      <div className="mt-12 pt-8 border-t border-white/10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 font-mono-tech text-xs">
          ...
          <div>
            <span className="text-white/40 block text-[10px] uppercase">STACK</span>
            <span className="font-bold text-[#355CFF]">{project2Data.stack.join(' · ')}</span>
          </div>
          <div>
            <span className="text-white/40 block text-[10px] uppercase">INFERENCE ARCH</span>
        ...
        <button
          ...
          onMouseLeave={() => setCursorMode('default')}
          className="group self-start lg:self-auto flex items-center gap-4 px-8 py-4 bg-white text-[#111111] font-display text-base sm:text-lg font-bold tracking-tight uppercase hover:bg-[#355CFF] hover:text-white transition-all duration-300"
        >
      ...
      <div className="mt-16 pt-4 border-t border-dashed border-white/20 flex items-center justify-between font-mono-tech text-[10px] text-white/50 uppercase">
        <span>RETURNING TO PAPER ENVIRONMENT // SCENE 03</span>
        <span className="text-[#355CFF]">↓ SCROLL DOWN TO ENTER OBLIQ INTELLIGENCE CHAMBER</span>
      </div>
    </section>
  );
};
